...
dups3<-flow_cleaned %>%
  ...
  tally() %>%
  arrange(desc(n))
dups3 #there are some duplicates in different rows
...
length(unique(as.character(N_load$permit_outfall))) #23 outfalls with/without locations
length(unique(as.character(N_load$key))) #2,454 keys
...
dim(N_load) #2454
length(unique(N_load$permit_outfall)) #23 outfalls with/without locations
length(unique(N_load$key)) #2454 keys

N_load_out <- left_join(N_load, outfall_locations, by = "permit_outfall")
dim(N_load_out) #2454

length(unique(as.character(N_load_out$permit_outfall))) #23 outfalls with locations
length(unique(as.character(N_load_out$key))) #2454 keys
