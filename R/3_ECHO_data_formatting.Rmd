...
length(unique(as.character(dat$permit_outfall))) #220 permit/outfalls
length(levels(as.factor(dat$key))) #46,661 keys
...
dup<-N_load_a %>%
  ...
  tally() %>%
  arrange(desc(n))
dup #there are no more duplicates
...
N_load_b$kg_N_sum_per_month <-
  ifelse(
    ...
    (
      N_load_b$kg_N_NH3_per_month + N_load_b$kg_N_NO3_per_month + N_load_b$kg_N_NO2_per_month + N_load_b$kg_N_NO3NO2_per_month 
    )
  )
...
N_load_a_2<-N_load_a %>%
  filter(is.na(kg_N_NO2_per_month) & is.na(kg_N_NO3NO2_per_month)) #selecting all other rows to rejoin
...
missing_huc8
#there are 10 outfalls with missing HUC8 assignments
write_csv(missing_huc8,
          file = here("ECHO_missing_huc8.csv"))
