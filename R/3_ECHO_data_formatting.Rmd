...
length(unique(as.character(N_load_a$permit_outfall))) #73 outfalls included here
...
dup<-keys_with_N %>% 
  ...
  filter(n>1)%>%
  arrange(desc(n))
dup #there are no longer flow rate duplicates
...
names(N_load_b)
length(unique(as.character(N_load_b$permit_outfall))) #23 outfalls included here
...
length(unique(as.character(N_load$permit_outfall))) #76 outfalls with/without locations
nrow(N_load) #8423
...
dim(N_load_CT_join) #25.486
dim(N_load) #8423
dim(CT_summary) #17,063
length(unique(as.character(N_load_CT_join$permit_outfall))) #156 outfalls with locations
length(unique(as.character(N_load_CT_join$key))) #25,486 keys
...
#join locations based on permit number and outfall number
dim(N_load_CT_join) #25.486
length(unique(N_load_CT_join$permit_outfall)) #156 outfalls with/without locations
length(unique(N_load_CT_join$key)) #25,486 keys


N_load_out <- left_join(N_load_CT_join, outfall_locations, by = "permit_outfall")
dim(N_load_out) #25.486
length(unique(as.character(N_load_out$permit_outfall))) #156 outfalls with locations
length(unique(as.character(N_load_out$key))) #24,486 keys
#the data are the same dimensions before and after the join
#check for NA's in latitude
summary(N_load_out$LATITUDE83) #492 missing values
...
dim(N_load_out) #25.486
dim(dat_join) #25.486
length(unique(as.character(dat_join$permit_outfall))) #156 outfalls with locations
length(unique(as.character(dat_join$key))) #25,486 keys
