...
#write_csv(N_load,here("data","ECHO_CT_data.csv"))
...
N_load<- N_load %>%
  mutate(month_year=format(as.Date(date), "%Y-%m")) %>%
  select(-date,-kg_N_sum_per_month)
...
dim(N_load_CT_join) #27038
dim(N_load) #9114
dim(CT_summary) #17,924
length(unique(as.character(N_load_CT_join$permit_outfall))) #157 outfalls with locations
length(unique(as.character(N_load_CT_join$key))) #26,177 keys
...
#join locations based on permit number and outfall number
dim(N_load_CT_join) #27,038
length(unique(N_load_CT_join$permit_outfall)) #157 outfalls with/without locations
length(unique(N_load_CT_join$key)) #26,177 keys


N_load_out <- left_join(N_load_CT_join, outfall_locations, by = "permit_outfall")
dim(N_load_out) #27038
length(unique(as.character(N_load_out$permit_outfall))) #157 outfalls with locations
length(unique(as.character(N_load_out$key))) #26177 keys
#the data are the same dimensions before and after the join
#check for NA's in latitude
summary(N_load_out$LATITUDE83) #560 missing values
...
dim(N_load_out) #27,038
dim(dat_join) #27,038
length(unique(as.character(dat_join$permit_outfall))) #157 outfalls with locations
length(unique(as.character(dat_join$key))) #26,177 keys
