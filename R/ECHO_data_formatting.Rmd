...
```{r}
#convert N_load_out to an sf object
no_na<-drop_na(N_load_out, LONGITUDE83)
N_load_out_sf<-st_as_sf(no_na, coords =c('LONGITUDE83','LATITUDE83'))

#read in huc8 file
huc8 = st_read(here('data','huc_8_dat_join','N_load_huc_join.shp'))

#set CRS of N_load_out to match the huc8 layer
st_crs(N_load_out_sf) <- st_crs(huc8)


joined = st_contains(N_load_out_sf,huc8)
...
# 
# #test
# HUC_comparison<-left_join(loc_comparison,HUC8_outfall_locations)
# 
# HUC_comparison$HUC_12_8<-paste0(0,substr(HUC_comparison$HUC_12,1,7))
# 
# HUC_comparison$HUC_dif<-as.numeric(HUC_comparison$HUC_12_8)-as.numeric(HUC_comparison$huc8)
# 
# SIC_desc<-HUC_comparison %>% group_by(SIC_CODES, SIC_DESCRIPTIONS) %>%
#   summarize(first(SIC_CODES))
# write.csv(SIC_desc, "SIC codes.csv", row.names = F)
# 
# #join locations based on permit number and outfall number
...
dim(joined) #53,599
length(unique(as.character(joined$permit_outfall))) #264 outfalls with locations
length(unique(as.character(joined$key))) #38,159 keys
