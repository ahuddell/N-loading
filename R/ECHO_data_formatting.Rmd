...
sample_dups<-flow_cleaned %>%
  filter(key %in% c(
    'CT0003263_001B_2011-06-30',
'CT0000086_001_2012-06-30',
'CT0000582_001_2003-08-31',
'CT0003875_001_2008-01-31',
'NY0007340_001_1998-02-28'
  ))

write.csv(sample_dups, 'sample_dups.csv',row.names = F)
...
names(N_load_b_wide)
length(unique(as.character(N_load_b_wide$permit_outfall))) #328 outfalls included here
...
names(N_load_b)
length(unique(as.character(N_load_b$permit_outfall))) #328 outfalls included here
...
length(unique(as.character(N_load_b_noNA$permit_outfall))) #18 outfalls included here
...
Exploring missingingness more

```{r}
col_specifications<-('ccffffffffffddff') #compact string to specify column types
dat2<-read_csv(file=here("data","Echo_LIS_complete_data.csv"),
               col_types = col_specifications)
dat2<-distinct(dat2) #remove a few duplicate rows
names(dat2)

##looking at missing values within parameters
param_NAs <-
  dat2 %>%
  select(c(param = 'Parameter', value = 'Reported Value')) %>%
  group_by(param) %>%
  summarize(missing = sum(is.na(value)),
            n = n()) %>%
  mutate(prop_missing = missing / n,
         prop_complete = (n - missing) / n)

ggplot(param_NAs, aes(y = param, x = prop_complete)) +
  geom_col() +
  xlab("proportion complete observations")
...
dat3 <- dat2 %>%
  ...
  filter(`Monitoring Location` == "1") %>% #this drops 158,423 rows and 99 permit/outfalls
  #removing infrequently used statistical base codes
  filter(`Statistical Base Long Desc` %in% c("Monthly Avg.","Average Value","Daily Avg.")) %>% #by this line, there are  474,995 rows dropped and 584 permit/outfalls dropped
  ...
  filter(!Parameter == "00625 - Nitrogen, Kjeldahl, total (as N)") %>%
  #by this line, there are 535,936 rows dropped  and 632 permit/outfalls dropped
...
missing_flow <- dat3 %>%
  group_by(key) %>%
  filter(!any(
    Parameter %in% c('00056 - Flow rate', '74076 - Flow', '00058 - Flow rate')
  ))
#~15k/70k observations are missing flow rate
...
#~36k/70k observations are missing N
