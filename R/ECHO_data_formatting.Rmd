...
dat <- dat %>%
  ...
  # omitting the few observations of these unclear units and Parameters
...
#evaluating how many observations report loads
load_report_tally<-dat%>%
  group_by(key)%>%
  tally(!is.na(kg_per_month))

sum((load_report_tally$n>0))/nrow(load_report_tally)*100
#only about 10% of DMRs report loads
...
dat$value<-  ifelse(
    dat$Parameter=="00608 - Nitrogen, ammonia dissolved", dat$value *14.0067/17.031, dat$value)

dat$value<- ifelse(
  dat$Parameter== "34726 - Nitrogen, ammonia, total (as NH3)", dat$value * 14.0067/17.031, dat$value)
...
test <- 
  ...
  select(key, permit, Outfall, date,param_simple, stat, days_per_month,
         mg_N_per_L, million_L_per_month, kg_per_month) %>%
  distinct()
...
Here we pivot the data wider and summarize duplicates by min, mean, and max. Then we calculate loads based on the concentrations and flow data, and remove the concentration and flow columns after we use them.

First, we report the detailed dataframes with all of the Parameter and Statistical Base Code Desc information. Then we report a simpler dataframe with fewer rows.
```{r}
...
max_vals_detailed<-dat %>%
  pivot_wider(names_from = param_simple, 
              values_from =c(mg_N_per_L, million_L_per_month,kg_per_month),
              values_fn = max) %>%
  select_if(not_all_na)

names(max_vals_detailed)

#evaluating the % of observations that report TN
sum(!is.na(max_vals_detailed$mg_N_per_L_TN))/nrow(max_vals_detailed)*100

#simpler dataframe with fewer rows
max_vals <- dat %>%
  select(!c(Parameter, `Monitoring Location`, Units, `Statistical Base Long Desc`, Column, day,days_per_month) ) %>%
  pivot_wider(
    names_from = param_simple,
    values_from = c(mg_N_per_L, million_L_per_month, kg_per_month),
    values_fn = max
  ) %>%
  select_if(not_all_na) %>%
...
max_vals <- max_vals %>%
  mutate(
    kg_N_TN_per_month = mg_N_per_L_TN * ifelse (
      is.na(million_L_per_month_flow_rate),
      million_L_per_month_flow_total,
      million_L_per_month_flow_rate
    )
  ) %>%
  mutate(
    kg_N_ON_per_month = mg_N_per_L_ON * ifelse (
      is.na(million_L_per_month_flow_rate),
      million_L_per_month_flow_total,
      million_L_per_month_flow_rate
    )
  ...
  mutate(
    kg_N_NH3_per_month = mg_N_per_L_NH3 * ifelse (
      is.na(million_L_per_month_flow_rate),
      million_L_per_month_flow_total,
      million_L_per_month_flow_rate
    )
  ...
  mutate(
    kg_N_NO2_per_month = mg_N_per_L_NO2 * ifelse (
      is.na(million_L_per_month_flow_rate),
      million_L_per_month_flow_total,
      million_L_per_month_flow_rate
    )
  ...
  mutate(
    kg_N_NO3_per_month = mg_N_per_L_NO3 * ifelse (
      is.na(million_L_per_month_flow_rate),
      million_L_per_month_flow_total,
      million_L_per_month_flow_rate
    )
  ...
  mutate(
    kg_N_IN_per_month = mg_N_per_L_IN * ifelse (
      is.na(million_L_per_month_flow_rate),
      million_L_per_month_flow_total,
      million_L_per_month_flow_rate
    )
  ...
  select(
    -mg_N_per_L_TN, -mg_N_per_L_ON, -mg_N_per_L_NH3, -mg_N_per_L_NO2, -mg_N_per_L_NO3, -mg_N_per_L_IN,
    -million_L_per_month_flow_rate,-million_L_per_month_flow_total, -stat
  )
...
min_vals <- min_vals %>%
  ...
  select(
    -mg_N_per_L_TN, -mg_N_per_L_ON, -mg_N_per_L_NH3, -mg_N_per_L_NO2, -mg_N_per_L_NO3, -mg_N_per_L_IN,
    -million_L_per_month_flow_rate,-million_L_per_month_flow_total,-million_L_per_month_flow_max_24h, -stat
  )

#############test case to demonstrate NAs
test2<-filter(max_vals, key=="CT0000086_1_1998-04-30")
test2

#code to remove rows with NAs in all of the value columns
cols_to_check = c("kg_per_month_TN","kg_per_month_NH3","kg_per_month_NO2", "kg_per_month_NO3", "kg_per_month_IN")
...
Evaluate how many rows there are per key. There are often 24--so we need to simplify the output further.
```{r}
max_vals%>%
  group_by(key) %>%
  tally()
