...
length(unique(as.character(N_load$permit_outfall))) #149 outfalls with/without locations
nrow(N_load) #11,797
...
#join locations based on permit number and outfall number
dim(N_load) #11,797
length(unique(N_load$permit_outfall)) #115 outfalls with/without locations
length(unique(N_load$key)) #11,383 keys

N_load_out <- left_join(N_load, outfall_locations, by = "permit_outfall")
dim(N_load_out) #44,626, 12

length(unique(as.character(N_load_out$permit_outfall))) #115 outfalls with locations
length(unique(as.character(N_load_out$key))) #11,383 keys
