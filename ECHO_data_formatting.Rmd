...
dat <- dat %>% mutate(
  million_L_per_month =
    case_when(
      ...
      Units == "(1X) - Gallons per Hour" ~ value / 10 ^ 6 * 3.785 * 24 * dat$days_per_month,
      ...
      Units == "(03) - Million Gallons per Day" ~ value * 3.785 * dat$days_per_month,
      # converts gallons to L and day to month
      Units == "(78) - Gallons per Minute" ~ value / 10 ^ 6 * 3.785 * 60 * 24 * dat$days_per_month,
...
Next, we reshape the data from long to wide format to get the mean, max, min, and 
totals for different parameters into different columns.
...
write.csv(test_case, "test_case.csv", row.names = F)
...
write.csv(max_vals, "ECHO_data_summarized_maximum_values.csv", row.names = F)
write.csv(min_vals, "ECHO_data_summarized_minimum_values.csv", row.names = F)
write.csv(mean_vals, "ECHO_data_summarized_mean_values.csv", row.names = F)
